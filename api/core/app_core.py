...
from api.scrapers.http_finviz import scrape_finviz
from api.scrapers.http_tradingview import scrape_tradingview
from api.scrapers.http_yahoo import scrape_yahoo
...
def create_app(settings: AppSettings, root_path: str = "") -> FastAPI:
...
def get_sources_status(settings: AppSettings) -> Dict[str, Dict[str, Any]]:
...
async def scrape_all_data(
    http_client: httpx.AsyncClient, settings: AppSettings, scrape_req: Optional[ScrapeRequest] = None
) -> Dict[str, Any]:
...
async def scrape_source(
    http_client: httpx.AsyncClient, source: str, settings: AppSettings, scrape_req: Optional[ScrapeRequest] = None
