...
async def scrape_all() -> Dict[str, Any]:
    ...
    try:
        ...
        async_tasks = [scrape_finviz(), scrape_yahoo()]
        ...
        # Log total results
        total_items = sum(sum(len(section) for section in source.values()) for source in data.values())
...
def send_data_to_api(data: Dict[str, Any]) -> bool:
    ...
    try:
        headers = {"Content-Type": "application/json", "User-Agent": "Financial-Scraper-Bot/1.0"}

        logger.info(f"📤 Enviando datos a {API_URL}")
        response = requests.post(API_URL, data=json.dumps(data, ensure_ascii=False), headers=headers, timeout=30)
